...
**Append results to today's plan file** (`user/plans/YYYY-MM-DD-Day.md`) as a `## Check-in HH:MM` section.
...
**NEVER read `user/raw/email-raw.json`** — it contains quarantined raw email content. Only the `email-triager` subagent reads it (ADR 008). If this skill needs email data, dispatch the subagent via the Agent tool; do not read the file directly.
