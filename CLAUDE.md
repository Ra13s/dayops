...
- Raw email output from `gather_email.py` is written to `user/raw/email-raw.json` via shell redirect
...
- Rule: `Read`/`cat`/pipe into context on `user/raw/email-raw.json` is prohibited
...
Each day's plan is saved to `user/plans/YYYY-MM-DD-Day.md` and appended throughout the day. This provides:
...
### File lifecycle
1. **Morning briefing** → creates `user/plans/2026-04-10-Fri.md` with the full plan
...
5. **Friday** → also creates `user/plans/2026-04-10-Fri-week-summary.md`
...
| `python scripts/gather_all.py --outlook-requests FILE --teams-requests FILE --email-raw-out user/raw/email-raw.json --date YYYY-MM-DD` | **All-in-one**: extracts tokens + runs calendar/todo/teams in parallel, quarantines raw email for subagent triage (~12s) | Combined JSON + quarantined email metadata |
